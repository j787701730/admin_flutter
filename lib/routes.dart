...
final routes = <String, WidgetBuilder>{
  ...
  '/staffDepartment': (_) => StaffDepartment(), // 组织架构 (完成)
  '/staffGroup': (_) => StaffGroup(), // 岗位架构 (完成)
  '/staffAdd': (_) => StaffAdd(), // 员工添加 (只能看看)
  '/csLogs': (_) => CsLogs(), // 软件日志 (完成)
  '/erpLogs': (_) => ErpLogs(), // Erp日志 (完成)
  '/mqLogs': (_) => MQLogs(), // 消息队列日志 (完成)
  '/wxLogs': (_) => WXLogs(), // 微信日志 (完成)
  '/smsLogs': (_) => SmsLogs(), // 短信日志 (完成)
  '/adminLogs': (_) => AdminLogs(), // 后台操作日志 (完成)
  '/optLogs': (_) => OPTLogs(), // 软件优化日志 (完成)
  '/chargeLogs': (_) => ChargeLogs(), // 对账日志 (完成)
  '/redpacketLogs': (_) => RedPacketLogs(), // 红包日志 (完成)
  '/cadLogs': (_) => CADLogs(), // CAD日志 (完成)
  '/phoneLogs': (_) => PhoneLogs(), // 通话日志 (完成)
  '/analysisLogs': (_) => AnalysisLogs(), // 日志分析日志 (完成)
  '/usersAdd': (_) => UsersAdd(), // 用户添加 (只能看看)
  '/usersCert': (_) => UsersCert(), // 实名认证 (完成)
  '/usersManager': (_) => UsersManager(), // 用户管理 (只能看看)
  '/balanceList': (_) => BalanceList(), // 资金账本 (完成)
  '/balancePricing': (_) => BalancePricing(), // 定价计划 (只能看看)
