...
class CADLogs extends StatefulWidget {
  @override
  _CADLogsState createState() => _CADLogsState();
}

class _CADLogsState extends State<CADLogs> {
  int curr_page = 1;
  int page_count = 20;
  Map param = {};
  List logs = [];
...
//  }
  ...
  getData({isRefresh: false}) async {
    ...
    Map data = {'curr_page': curr_page, 'page_count': page_count, 'param': jsonEncode(param)};

    if (defaultVal != 'all') {
      data['order'] = defaultVal;
    }

    ajax('Adminrelas-Logs-CADLogs', data, true, (res) {
      if (mounted) {
        setState(() {
          logs = res['data'];
